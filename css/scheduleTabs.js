...
function genSchedule() {
	$.getJSON(server + "/json/events.json", function (json) {
		...
		events.sort(function(a, b) {
			return parseFloat(a.time_start) - parseFloat(b.time_start);
		});
		events.sort(function(a, b) {
...
function genEventCardDate(obj) {
	console.log(!!$('#card-date-' + obj.id));
	console.log(('#cards-' + obj.date.replace(/[.]/gi,'-')));
	console.log(obj.id);
	console.log(obj.trek_color);
	console.log(obj.time_start);
	console.log(obj.time_end);
	console.log(obj.date);
	console.log(obj.partner_logo.replace(/;/g,''));
...
function genEventCardTrek(obj) {
	...
	$('#cards-trek-' + obj.trek_id).append('<div id=\"card-trek-' + obj.id + '\" class=\"programm-card\" style="border-left-color: ' + obj.trek_color + ';" onclick=\"changeEvent(' + obj.id + ')\"><div class="programm-card__favorite"><svg width="16" height="17" class="ico-svg"><use xlink:href="./img/svg/ico-star.svg#ico-star"></use></svg><span class="favorite-on">В избранном</span><span class="favorite-off">В избранноe</span></div><div class="programm-card__time"><img src="./img/clock.svg" width="16" height="16" alt=""><div>' + obj.time_start + ' - ' + obj.time_end + '<div class="programm-card__date">' + obj.date + '</div></div></div><div class="programm-card__body"><div><div class="programm-card__category" style="color:' + obj.trek_color + ';">' + obj.trek + '</div><a class="programm-card__title modal-toggle-' + obj.id + '\" onclick=\"event.stopPropagation(); showHideModal(' + obj.id + '); window.scrollTo(0,0);\">' + obj.title + '</a><div class="programm-card__address"><i class="fa fa-map-marker" aria-hidden="true"></i>' + obj.hall + '</div></div><div class=\"programm-card__logo\"><div class=\"swiper swiper-container-horizontal\"><div class=\"swiper-wrapper\">' + obj.partner_logo.replace(/;/g,'') + '</div><div class=\"swiper-pagination\"></div></div></div></div></div>');
