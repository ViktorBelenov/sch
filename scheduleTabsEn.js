...
function genEventCardDate(obj) {
	...
	$('#cards-' + obj.date_en.replace(/[\/]/gi,'-')).append('<div id=\"card-date-' + obj.id + '\" class=\"programm-card\" style="border-left-color: ' + obj.trek_color + ';" onclick=\"changeEvent(' + obj.id + ')\"><div class="programm-card__favorite"><svg width="16" height="17" class="ico-svg"><use xlink:href="../img/svg/ico-star.svg#ico-star"></use></svg><span class="favorite-on">Favorite</span><span class="favorite-off">To favorite</span></div><div class="programm-card__time"><img src="../img/clock.svg" width="16" height="16" alt=""><div>' + obj.time_start + ' - ' + obj.time_end + '<div class="programm-card__date">' + obj.test_time_en + '</div></div></div><div class="programm-card__body"><div><div class="programm-card__category" style="color:' + obj.trek_color + ';">' + obj.trek_en + '</div><a class="programm-card__title modal-toggle-' + obj.id + '\" onclick=\"event.stopPropagation(); showHideModal(' + obj.id + '); window.scrollTo(0,0);\">' + obj.title_en + '</a><div class="programm-card__address"><i class="fa fa-map-marker'+ `${obj.hall!=''? '':'disabled'}` + '" aria-hidden="true"></i>' + `${obj.hall_en!=''? obj.hall_en :''}` + '</div></div><div class=\"programm-card__logo\"><div class=\"swiper swiper-container-horizontal\"><div class=\"swiper-wrapper\">' + obj.partner_logo_en.replace(/;/g,'') + '</div><div class=\"swiper-pagination\"></div></div></div></div></div>');
...
function genEventPopup(obj) {
	...
	$('.page').append('<div id=\"event-popup-' + obj.id + '\" class=\"modal\"><div class=\"modal-overlay modal-toggle-' + obj.id + '\" onclick=\"showHideModal(' + obj.id + ')\"></div><div class=\"modal-wrapper modal-transition\"><div class=\"modal-inner\"><button class=\"modal-close modal-toggle-' + obj.id + '\"  onclick=\"showHideModal(' + obj.id + ')\"><img src=\"../img/close.svg\" alt=\"\"></button><div class=\"modal-body\"><div class=\"event-modal\"><div id=\"add-' + obj.id + '\" onclick=\"changeEvent(' + obj.id + ')\" class=\"event-modal__add-favorite\"><svg width="16" height="17" class="ico-svg"><use xlink:href="../img/svg/ico-star.svg#ico-star"></use></svg><span class="favorite-on">Favorite</span><span class="favorite-off">To favorite</span></div><div class=\"event-modal__title\">' + obj.title_en + '</div><div class=\"event-modal__body\"><div><div class=\"event-modal__category\" style="display:none;">Trek</div><div class=\"event-modal__subtitle\" style=\"color:' + obj.trek_color + ';\">' + obj.trek_en + '</div><div class=\"event-modal__info\"><div class=\"event-modal__time\"><i class=\"fa fa-clock-o\" aria-hidden=\"true\"></i><div>' + obj.test_time_en + '<br>' + obj.time_start + ' - ' + obj.time_end + '</div></div><div class=\"event-modal__address\"><i class="fa fa-map-marker'+ `${obj.hall_en!=''? '':'disabled'}` + '" aria-hidden="true"></i>' + `${obj.hall_en!=''? obj.hall_en :''}` + '</div></div>' + obj.description_en + '</div><div class=\"event-modal__partner\"><div class=\"event-modal__partner__title'+`${obj.partner_img!=''? '':' disable'}`+ '\">Partner</div><div class=\"event-modal__partner__list\">' + obj.partner_img_en.replace(/;/g,'') + '</div></div></div></div></div></div></div></div>');
}
